...
pause=60
attempt=0
while [[ "$(curl -u admin:${generic_password} -k -s -o /dev/null -w '%{http_code}' https://${ip_nsx_vip_vip}/api/v1/cluster/status)" != "200" ]]; do
...
pause=60
attempt=0
while [[ "$(curl -u admin:${generic_password} -k -s  https://${ip_nsx_vip_vip}/api/v1/cluster/status | jq -r .detailed_cluster_status.overall_status)" != "STABLE" ]]; do
...
echo "NSX Manager ready at https://${ip_nsx_vip_vip}"
if [ -z "${SLACK_WEBHOOK_URL}" ] ; then echo "ignoring slack update" ; else curl -X POST -H 'Content-type: application/json' --data '{"text":"'$(date "+%Y-%m-%d,%H:%M:%S")', '${basename_sddc}': NSX Manager ready at https://'${ip_nsx_vip_vip}'"}' ${SLACK_WEBHOOK_URL} >/dev/null 2>&1; fi
#
# Transport Zones
#
...
do
  ...
  /bin/bash /home/ubuntu/nsx/set_object.sh "${ip_nsx_vip_vip}" "${generic_password}" \
