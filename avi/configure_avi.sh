  ...
  do
    ...
    count_dns=$((count_dns+1))
      if [[ "${count_dns}" -eq 120 ]]; then
        log_message "$(date "+%Y-%m-%d,%H:%M:%S"), nested-${basename_sddc}: ERROR: Unable to get the DNS VS UP after ${count_dns} attempts of ${pause_dns} seconds" "${log_file}" "${slack_webhook}" "${google_webhook}"
        exit 100
      fi
  done
  log_message "$(date "+%Y-%m-%d,%H:%M:%S"), nested-${basename_sddc}: DNS VS UP after ${count_dns} attempts of ${pause_dns} seconds" "${log_file}" "${slack_webhook}" "${google_webhook}"
fi
#
